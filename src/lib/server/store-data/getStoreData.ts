...
const buildCategoriesFromMenus = (storeId: number, menus: Menu[]): MenuCategory[] => {
...
export const fetchMockStoreData = async (storeId: number): Promise<StoreDataPayload> => {
  const store = getMockStore(storeId);
  const menus = getMockMenus(storeId)
  ...
  const categories = buildCategoriesFromMenus(storeId, menus);
...
export const fetchStoreDataFromSupabase = async (storeId: number): Promise<StoreDataPayload | null> => {
  const supabase = createSupabaseClient();

  const { data: storeRow, error: storeError } = await supabase
    .from("stores")
    .select(
      "store_id, name, description, address, phone, logo_url, cover_url, business_hours, notice, updated_at",
    )
    .eq("store_id", storeId)
    .limit(1)
    .maybeSingle();
  ...
  const { data: menuRows, error: menuError } = await supabase
    .from("menus")
    .select(
      "menu_id, store_id, name, description, price, image_url, is_active, category_id, allergy_info, display_order, updated_at",
    )
    .eq("store_id", storeId)
    .eq("is_active", true)
    .order("display_order", { ascending: true, nullsFirst: true })
